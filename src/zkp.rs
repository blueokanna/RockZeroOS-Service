...
impl ZkpContext {
    ...
    pub fn generate_password_proof(&self, password: &str) -> Result<PasswordProofData, AppError> {
        ...
        let challenge_hash = blake3_hash!(
        ...
        challenge_bytes.copy_from_slice(challenge_hash.as_bytes());
    ...
    pub fn verify_password_proof(
        ...
        let expected_challenge = blake3_hash!(
            commitment_point.compress().as_bytes(),
            blinding_commitment_point.compress().as_bytes(),
            PROOF_LABEL
        );
        ...
        expected_challenge_array.copy_from_slice(expected_challenge.as_bytes());
...
mod tests {
    ...
    fn test_password_proof() {
        ...
        // generate_password_proof 会生成一个新的承诺
        // 验证时需要使用证明中包含的承诺
    ...
    fn test_enhanced_proof() {
        let ctx = ZkpContext::new();
        let password = "SecurePassword123!@#$%";
        // generate_enhanced_proof 内部调用 generate_password_proof
        // 验证时需要使用证明中包含的承诺
