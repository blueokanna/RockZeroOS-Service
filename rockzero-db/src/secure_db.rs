...
impl ReedSolomon {
    ...
    pub fn reconstruct(&self, shards: &mut [Option<Vec<u8>>]) -> Result<Vec<u8>, AppError> {
        ...
        let selected: Vec<usize> = available_indices
            .iter()
...
impl RecoveryData {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, AppError> {
        ...
        for _ in 0..parity_count {
            ...
            parity_crcs.push(u32::from_le_bytes(
                bytes[offset..offset + 4].try_into().unwrap(),
            ));
            offset += 4;
        }
...
impl SecureDatabase {
    ...
    async fn read_recovery_data(&self, block_id: u64) -> Result<RecoveryData, AppError> {
        ...
        let mut file =
            File::open(&self.recovery_path).map_err(|e| AppError::IoError(e.to_string()))?;
        ...
        Err(AppError::NotFound(
            "Recovery data not found for block".to_string(),
        ))
    }
    ...
    pub async fn load(&self) -> Result<(), AppError> {
        ...
        let mut file = File::open(&self.db_path).map_err(|e| AppError::IoError(e.to_string()))?;
    ...
    pub async fn stats(&self) -> DatabaseStats {
        ...
        let total_size: usize = blocks.values().map(|b| b.encrypted_data.len()).sum();
